...
def pais_mais_cidades(df):
    df_aux = (df.loc[:, ['country', 'city']]
                ...
                .reset_index())
    df_aux.columns = ['Países', 'Qtde_Cidades']
    figura = px.bar(df_aux, x = 'Países', y = 'Qtde_Cidades', text_auto = 'Qtde_Cidades')
    return figura
...
def pais_mais_restaurantes(df):
    df_aux = (df.loc[:, ['country', 'restaurant_id']]
                ...
                .reset_index())
    df_aux.columns = ['Países', 'Qt_Restaurantes']
    figura = px.bar(df_aux, x = 'Países', y = 'Qt_Restaurantes', text_auto = 'Qt_Restaurantes')
    return figura
...
def pais_maior_media_avaliacoes(df):
    df_aux = (df.loc[:, ['country', 'aggregate_rating']]
                ...
                .reset_index())
    df_aux.columns = ['Países', 'Qt_Avaliações']
    figura = px.bar(round(df_aux, 2), x = 'Países', y = 'Qt_Avaliações', text_auto = 'Qt_Avaliações')
    return figura
...
def media_preco_prato_2(df):
    df_aux = (df.loc[:, ['country', 'average_cost_for_two']]
                ...
                .reset_index())
    df_aux.columns = ['Países', 'Qt_Média_prato_para_2_pessoas']
    figura = px.bar(round(df_aux, 2), x = 'Países', y = 'Qt_Média_prato_para_2_pessoas', text_auto = 'Qt_Média_prato_para_2_pessoas')
    return figura
...
def tipo_culinaria_maior_nota_media(df):
    df_aux = (df.loc[:, ['cuisines', 'aggregate_rating']]
                ...
                .sort_values('aggregate_rating', ascending = False)
                .reset_index())
    figura =  px.bar(round(df_aux, 2), x = 'cuisines', y = 'aggregate_rating', text_auto = 'aggregate_rating')
    return figura
...
linhas_selecionadas = df['country'].isin(paises)#isin está em
...
with st.container():
    ...
    with st.container():                
        st.markdown('##### Quantidade de países cadastrados')
        figura = pais_mais_restaurantes(df)
        st.plotly_chart(figura, use_container_width = True)  

    with st.container():                
        st.markdown('##### Quantidade de cidades por país')
        figura = pais_mais_cidades(df)
        st.plotly_chart(figura, use_container_width = True)  
    ...
    with st.container(): 
        ...
        with col1:
            ...
            figura = pais_maior_media_avaliacoes(df)
            st.plotly_chart(figura, use_container_width = True)  
        with col2:
            st.markdown('##### Média de preço de um prato para 2 pessoas por país')
            figura = media_preco_prato_2(df)
            st.plotly_chart(figura, use_container_width = True)  
