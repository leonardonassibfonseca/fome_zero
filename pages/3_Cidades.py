...
with st.container():
    ...
    with st.container():                
        st.markdown('##### Top 10 cidades com mais restaurantes na base de dados por país')

        figura = cidade_mais_restaurantes(df)
    ...
    with st.container(): 
        ...
        with col1:
            st.markdown('##### Cidades c/ restaurantes c/ média de avaliação acima de 4 por país')            
            figura = cidade_mais_restaurantes_nota_maior_4(df)
            st.plotly_chart(figura, use_container_width = True)  
        with col2:
            st.markdown('##### Cidades c/ restaurantes c/ média de avaliação abaixo 2.5 por país')
            figura = cidade_mais_restaurantes_nota_menor_2_5(df)
            st.plotly_chart(figura, use_container_width = True)  
    
    with st.container():                
        st.markdown('##### Top 10 cidades com mais restaurantes com tipos culinários distintos por país')
        figura = cidade_maior_qtde_tipo_culinario(df)
        st.plotly_chart(figura, use_container_width = True)  