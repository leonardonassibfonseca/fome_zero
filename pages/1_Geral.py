...
with st.container():
    ...
    with col1:
        qtde_paises = df['country'].nunique()
        col1.metric('Países cadastrados', qtde_paises)
    with col2:
        qtde_cidade = df['city'].nunique()
        col2.metric('Cidades cadastrados', qtde_cidade)
    with col3:
        qtde_restaurantes = df['restaurant_id'].nunique()
        qtde_restaurantes = '{:,}' .format(qtde_restaurantes)
        qtde_restaurantes = qtde_restaurantes.replace(',', '.')
        col3.metric('Restaurantes cadastrados', qtde_restaurantes)
    with col4:
        qtde_avaliacoes = df['votes'].sum()
        qtde_avaliacoes = '{:,}' .format(qtde_avaliacoes)
        qtde_avaliacoes = qtde_avaliacoes.replace(',', '.')
        col4.metric('Avaliações feitas na plataforma', qtde_avaliacoes)
    with col5:
        tipos_culinarios = df['cuisines'].nunique()
        col5.metric('Tipos de culinária oferecidas', tipos_culinarios)
